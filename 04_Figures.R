...
################################################################################
## Scale per Sample Size: N=10,000 for main body of text 
################################################################################
# Define parameter combinations
mechanisms <- c("MCAR", "MAR", "MNAR")
missing <- c("0.25","0.5","0.75")
...
AUCmin <- signif(min(n10000_combined$AUC), digits = 2) -0.02
AUCmax <- signif(max(n10000_combined$AUC), digits=2) + 0.02

BIASmin <- signif(min(n10000_combined$bias), digits = 2) -0.02
BIASmax <- signif(max(n10000_combined$bias), digits = 2) +0.02
...
RMSEmin <- signif(min(n10000_combined$rmse), digits = 2) -0.02
RMSEmax <- signif(max(n10000_combined$rmse), digits = 2)+0.02
...
for (mech in mechanisms) {
  ...
  for (measure_name in names(measures)) {
    ...
    # Add measure-specific customizations
    p <- p + switch(measure_name,
                    "AUC" = scale_x_continuous(limits = c(AUCmin, AUCmax), breaks = seq(0, 1, by=0.02)),
                    "Brier" = scale_x_continuous(limits = c(BRIERSCmin, BRIERSCmax), breaks = seq(-0.05, 0.3, by=0.02)),
                    "Bias" = list(
                      scale_x_continuous(limits = c(BIASmin, BIASmax), breaks = seq(-0.06, 0.06, by=0.01)),
                      geom_vline(xintercept = 0, linetype = "dashed", colour = "grey30")
                    ),
                    "Cal_Int" = list(
                      scale_x_continuous(limits = c(CALINTmin, CALINTmax), breaks = seq(-1, 1, by=0.1)),
                      geom_vline(xintercept = 0, linetype = "dashed", colour = "grey30")
                    ),
                    "Cal_Slope" = list(
                      scale_x_continuous(limits = c(CALSLOPEmin, CALSLOPEmax), breaks = seq(0.50, 2, by=0.05)),
                      geom_vline(xintercept = 1, linetype = "dashed", colour = "grey30")
                    ),
...
for (mech in mechanisms) {
  ...
  for (measure_name in names(measures)) {
    plot_name <- paste(mech, "10000", measure_name, sep = "_")
    plots_n10000[[mech]][[measure_name]] <- all_plots[[plot_name]]
  }
}
...
for (mech in mechanisms) {
  ...
  # Calibration plots (keep y-axis)
  cal_int_plot   <- plots_n10000[[mech]][["Cal_Int"]] + theme(legend.position = "none")
  cal_slope_plot <- plots_n10000[[mech]][["Cal_Slope"]] + theme(legend.position = "none")
  
  # Other plots (strip y-axis and legend)
  auc_plot   <- plots_n10000[[mech]][["AUC"]]   + theme(legend.position = "none", axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
  brier_plot <- plots_n10000[[mech]][["Brier"]] + theme(legend.position = "none", axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
  bias_plot  <- plots_n10000[[mech]][["Bias"]]  + theme(legend.position = "none", axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
  rmse_plot  <- plots_n10000[[mech]][["RMSE"]]  + 
    theme(legend.position="bottom", axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
  ...
  combined_plot <- (cal_int_plot / cal_slope_plot | auc_plot + brier_plot + bias_plot + rmse_plot) +
    plot_layout(guides = "collect", widths = c(1, 2)) +
    plot_annotation(
      title = paste("Model Performance Metrics -", mech, "Mechanism (N=10,000)"),
      theme = theme(
        legend.position = "bottom",
        plot.title = element_text(size = 20, face = "bold", hjust = 0.5))
    )
  ...
  ggsave(
    filename = paste0(figures, "/", mech, "_N10000_patchwork.pdf"),
    plot = combined_plot,
    width = 14, height = 10
  )
...
################################################################################
## Scale per Sample Size: N=500 for main body of text 
################################################################################
# Initialize empty list to store all plots
all_plots <- list()
...
BIASmin <- signif(min(n500_combined$bias), digits = 2) -0.02
BIASmax <- signif(max(n500_combined$bias), digits = 2) +0.02
...
RMSEmin <- signif(min(n500_combined$rmse), digits = 2) -0.02
RMSEmax <- signif(max(n500_combined$rmse), digits = 2)+0.02
...
for (mech in mechanisms) {
  ...
  for (measure_name in names(measures)) {
    plot_name <- paste(mech, "500", measure_name, sep = "_")
    plots_n500[[mech]][[measure_name]] <- all_plots[[plot_name]]
  }
}
...
for (mech in mechanisms) {
  ...
  # Calibration plots (keep y-axis)
  cal_int_plot   <- plots_n500[[mech]][["Cal_Int"]] + theme(legend.position = "none")
  cal_slope_plot <- plots_n500[[mech]][["Cal_Slope"]] + theme(legend.position = "none")
  
  # Other plots (strip y-axis and legend)
  auc_plot   <- plots_n500[[mech]][["AUC"]]   + theme(legend.position = "none", axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
  brier_plot <- plots_n500[[mech]][["Brier"]] + theme(legend.position = "none", axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
  bias_plot  <- plots_n500[[mech]][["Bias"]]  + theme(legend.position = "none", axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
  rmse_plot  <- plots_n500[[mech]][["RMSE"]]  + 
    theme(legend.position="bottom", axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
  ...
  combined_plot <- (cal_int_plot / cal_slope_plot | auc_plot + brier_plot + bias_plot + rmse_plot) +
    plot_layout(guides = "collect", widths = c(1, 2)) +
    plot_annotation(
      title = paste("Model Performance Metrics -", mech, "Mechanism (N=500)"),
      theme = theme(
        legend.position = "bottom",
        plot.title = element_text(size = 20, face = "bold", hjust = 0.5))
    )
  ...
  ggsave(
    filename = paste0(figures, "/", mech, "_N500_patchwork.pdf"),
    plot = combined_plot,
    width = 14, height = 10
  )
