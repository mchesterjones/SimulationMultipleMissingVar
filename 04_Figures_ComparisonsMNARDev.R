...
## Datasets 
################################################################################
load(paste0(val_data,"CompatabilityofMechanisms_MNARDev_LongFormat.Rdata"))
load(paste0(val_data,"ComtabilityofMechanisms_MNARDev_AllIterations.Rdata"))
...
bias_comp <- ggplot(df_long %>% filter(Measure == "bias" & 
 ...
 # geom_errorbar(aes(xmin = LCI, xmax = UCI), width = 0.8) +
  labs(x = "Bias of predictions vs true outcome",
       ...
       colour = "Method\n(Mean, 95% CI)") +
  theme_minimal() +
  scale_x_continuous(limits=c(-0.02,0.01), breaks=seq(-0.06,0.06, by=0.005))+
...
ggsave(
  filename = paste0(figures, "Compatability of Mechanisms MNAR Development.pdf"),
  plot = patchwork,
  width = 14, height = 10
)
ggsave(
  filename = paste0(figures, "Compatability of Mechanisms MNAR Development.svg"),
  plot = patchwork,
  width = 14, height = 10
)
